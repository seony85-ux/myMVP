...
* 감정 점수 선택 (1~5 단계)
* BGM 선택 (2개 옵션 + 없음)
* 루틴 모드 선택 (단계별 가이드 / 자율모드, 기본값: 단계별 가이드)
...
* **루틴 모드 선택**: 버튼 UI (단계별 가이드 / 자율모드), 기본값 단계별 가이드
* **스킨케어 단계**: 자율모드에서는 숨김, 단계별 가이드 모드에서만 선택 가능. 최소 1개 선택 유도
...
* **레이아웃**: 화면을 상·하 절반으로 구분. 상단 절반에 토글 영역, 하단 절반에 팁 박스 배치
* **질문 텍스트**: "음성 가이드를 들으시겠어요?" (질문과 토글 사이 적절한 간격 유지)
* **토글 UI**: 상태별 안내 문구 표시 (On: "음성과 텍스트 가이드가 같이 재생됩니다." / Off: "텍스트 가이드만 표시됩니다")
* **팁 박스**: CTA 버튼 바로 위 위치. 제목 "💡 Tip", 본문은 온오프 안내와 동일한 글씨 크기(text-base sm:text-lg). 배경/테두리로 시각적 구분
* **팁 본문**: "루틴을 진행하는 도중 딴생각이 들어도 괜찮아요. 그런 나를 그저 조용히 알아차리고, 생각이 흘러가도록 가만히 지켜보세요. 온전히 나에게만 집중하는 가장 편안한 시간이 되기를 바랍니다."
...
* **만족도**: 선택형 UI, 미선택 시 에러 메시지 표시
* **하단**: [완료] 버튼, 만족도 선택 시 활성화

#### 4.6.4 데이터 요구사항

* 만족도(1~5), 감정 변화량, 중단 여부
...
* 감사 문구 + 이미지 표시
* “더 알아보기” 버튼 클릭 시 외부 링크(서비스 소개 페이지) 새 탭 열기
* 뒤로가기 시 “이미 양식이 제출되었습니다” 모달 표시, [확인] 클릭 시 인트로로 이동

#### 4.7.3 UI/UX 요구사항

* **이미지**: 원형 이미지 영역
* **텍스트**: “체험해주셔서 감사합니다” 및 서브 문구(“매일 조금씩, 나에게 집중하는 순간을...”)
* **버튼**: [더 알아보기] (ghost 스타일)
...
5. 감정 변화 입력 → [결과 보기]
6. 결과 요약 → [완료]
7. 감사 화면 → [더 알아보기] 또는 뒤로가기 시 모달 확인 후 인트로 이동
